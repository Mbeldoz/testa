...
local DataLoader = {} do
    ...
    function DataLoader:Once()
        ...
        for _, item in pairs(player.PlayerGui.CosmeticShop_UI.CosmeticShop.Main.Holder.Shop:GetDescendants()) do
            if item:IsA("Frame") then
                if item.Parent.Name == "BottomSegment" or item.Parent.Name == "TopSegment" then
                    table.insert(Table.LoadStock.cosmetic, item.Name)
                end
            end
        end
...
local GameModule = {} do
        ...
        for _, prompt in pairs(garden.Important.Plants_Physical:GetDescendants()) do
            if prompt:IsA("ProximityPrompt") and prompt.Parent and prompt.Parent:IsA("BasePart") then
                ...
                local function collect(obj)
                    if mode =="Aura (For Bad Device)" then
                ...
                if mode == "Aura (For Bad Device)" then
                    ...
                    local prompt_dis = prompt.MaxActivationDistance * 2
                    if distance <= prompt_dis then
                        if Config.FilterBool then
                            if Config.FilterMode == "Match All" then
                                list = GameModule:FruitMatch(fruit)
                            else
                        ...
                        if not fruit:GetAttribute("Favorited")and not Config.Busy_Event then
                            if Config.FilterBool and Config.FilterMode == "Blacklist" and not list then
                                print(Config.FilterMode)
                                collect(prompt)
                            elseif Config.FilterBool and Config.FilterMode == "Whitelist" and list then
                                print(Config.FilterMode)
                                collect(prompt)
                            elseif Config.FilterBool and Config.FilterMode == "Match All" and list then
                                print(Config.FilterMode)
                                collect(prompt)
                            elseif not Config.FilterBool or Config.FilterBool == false then
                                print("No filter")
                                collect(prompt)
                            end
                        end
    ...
    function GameModule:placeRandomEgg()
        ...
        while attempt < maxAttempts do
            ...
            local safeX = (GARDEN_SIZE_X - ITEM_SIZE) / 2
            local safeZ = (GARDEN_SIZE_Z - ITEM_SIZE) / 2

            local randomX = gardenCenter.X + math.random(-safeX*100, safeX*100)/100
            local randomZ = gardenCenter.Z + math.random(-safeZ*100, safeZ*100)/100
        ...
        warn("Could not find valid position after", maxAttempts, "attempts")
        return false
    end
    ...
    function GameModule:MoveTree(parent)
        ...
        for _, child in ipairs(parent.Important.Plants_Physical:GetChildren()) do
            if child.Name:find(Config.MoveTreeTarget:gsub("%s*%(x%d+%)", "")) then
                ...
                task.wait()
                character.HumanoidRootPart.CFrame = CFrame.new(child:FindFirstChildOfClass("Part").CFrame.Position + Vector3.new(0, 3, 0))
    ...
    function GameModule:RemoveTree(parent)
        for _, child in ipairs(parent.Important.Plants_Physical:GetChildren()) do
            local cleanTargetName = Config.RemoveTreeTarget:gsub("%s*%(x%d+%)", "")
            local cleanChildName = child.Name:gsub("%s*%(x%d+%)", "")
            if cleanChildName == cleanTargetName then
                ...
                for _, item in ipairs(player.Backpack:GetChildren()) do
                    if item:GetAttribute("ItemName") == tool then
                        ...
                        if Config.SavePos2 then
                            print("SavePos")
                            local position = CFrame.new(math.floor(Config.SavePos2[1]), 0.135525137, math.floor(Config.SavePos2[2]), 0.064701885, 0.857260585, -0.510801494, -0, 0.51187402, 0.859060585, 0.997904658, -0.05558284, 0.0331192128)
    ...
    while _G.NoLag and task.wait(1) do
        ...
        if Config.AutoBuyEvent and #Config.WhitelistEvent > 0 and workspace:GetAttribute("NightEvent") then
            ...
            for _, seedName in ipairs(Config.WhitelistEvent) do
                local seedData = stockData[seedName]
                if seedData and seedData.count > 0 and cash >= seedData.price then
                    GameEvents.BuyNightEventShopStock:FireServer(seedName)
                    task.wait()
                end
            end
        end
        ...
        if Config.AutoCosmeticBuy and #Config.BuyCosmeticList > 0 then
            for _, seedName in ipairs(Config.BuyCosmeticList) do
                GameEvents.BuyCosmeticItem:FireServer(seedName)
                task.wait()
            end
        end
    end
...
Window:Dialog({
    ...
    do
        ...
        local serVer = Tabs.Main:AddInput("serVer", {
            Title = "Input Server Version",
            Description = "Hopping to server verison that less or equal",
            Numeric = true,
            Callback = function(Value)
                if Value then
                    BaseVar.ServerVersion = Value
                end
            end
        })
        ...
        ServerHopper:OnChanged(function()
            ...
            if BaseVar.ServerHopper == true then
                task.spawn(function()
                    ...
                    if tonumber(version) > tonumber(BaseVar.ServerVersion) then
                        BaseModule:hopServer()
                    end
                end)
            end
    ...
    do
        ...
        local Section = Tabs.Utility:AddSection("Night Event & Blood Moon")
        ...
        local EventShop = Tabs.Utility:AddDropdown("EventShop", {
            ...
            Description = "Auto buy item Twilight",
            Values = {},
            Multi = true,
            Default = {""},
        })
        ...
        local EventShop2 = Tabs.Utility:AddDropdown("EventShop2", {
            Title = "Stock Whitelist",
            Description = "Auto buy item Bloodmoon",
            Values = {},
            Multi = true,
            Default = {""},
        })

        EventShop2:OnChanged(function(Value)
            Config.WhitelistEvent2 = {}
            for name, _ in pairs(Value) do
                table.insert(Config.WhitelistEvent2, name)
            end
        end)

        local AutoBuyEvent2 = Tabs.Utility:AddToggle("AutoBuyEvent2", {Title = "Auto Buy Blood Moon", Default = false })
        AutoBuyEvent2:OnChanged(function()
            Config.AutoBuyEvent2 = Options.AutoBuyEvent2.Value
        end)
        ...
        task.spawn(function ()
            ...
            EventShop2:SetValues(Table.LoadStock.event2)
        end)
    end
    ...
    do
        ...
        local AutoBuy = Tabs.Shop:AddToggle("AutoBuy", {Title = "Auto Buy Seed", Default = false })
        ...
        local AutoGearBuy = Tabs.Shop:AddToggle("AutoGearBuy", {Title = "Auto Buy Gear", Default = false })
        AutoGearBuy:OnChanged(function()
            Config.AutoBuyGear = Options.AutoGearBuy.Value
        end)


        local MultiStock3 = Tabs.Shop:AddDropdown("MultiStock3", {
            Title = "Stock Whitelist",
            Description = "Auto buy item Whitelist",
            Values = {},
            Multi = true,
        ...
        MultiStock3:OnChanged(function(Value)
            Config.BuyCosmeticList = {}
            for name, _ in pairs(Value) do
                table.insert(Config.BuyCosmeticList, name)
            end
        end)


        local AutoCosmeticBuy = Tabs.Shop:AddToggle("AutoCosmeticBuy", {Title = "Auto Buy Cosmetic", Default = false })
        AutoCosmeticBuy:OnChanged(function()
            Config.AutoCosmeticBuy = Options.AutoCosmeticBuy.Value
        end)
        ...
        task.spawn(function ()
            ...
            MultiStock3:SetValues(Table.LoadStock.cosmetic)
        end)
    end

    --[Experimential]
    do
        local Section = Tabs.Experimential:AddSection("Vuln (Patch)")

        local DupeMulti = Tabs.Experimential:AddSlider("DupeMulti", {
            Title = "Dupe Speed Multipier",
            Default = 1,
            Min = 1,
            Max = 20,
            Rounding = 1,
            Callback = function(Value)
                ...
                Config.DupeMulti = val
            end
        })


        local Dupe2 = Tabs.Experimential:AddToggle("Dupe2", {Title = "Inf Money", Description = "Need Someone Equip Expensive Pet",Default = false })
        Dupe2:OnChanged(function()
            Config.Dupe2 = Options.Dupe2.Value
            if Config.Dupe2 then
                task.spawn(function()
                    local peoplePet
                    if peoplePet == nil then
                        for _, pet in pairs(workspace:GetDescendants()) do
                            if pet:IsA("Tool") and pet:GetAttribute("ItemType") == "Pet" and pet:GetAttribute("OWNER") ~= playername then
                                peoplePet = pet
                                break
                            end
                        end
                    end
                    for i = 1, Config.DupeMulti do
                        task.spawn(function()
                            while Config.Dupe2 and _G.NoLag and task.wait(0.1) do
                                if peoplePet then
                                    local SellPet = GameEvents.SellPet_RE
                                    SellPet:FireServer(peoplePet)
                                end
                            end
                        end)
                        task.wait()
        ...
        local Unobtainz = Tabs.Experimential:AddToggle("Unobtainz", {Title = "Buy Banana", Default = false })
        Unobtainz:OnChanged(function()
            Config.Unrelease = Options.Unobtainz.Value
        end)

        Tabs.Experimential:AddButton({
            Title = "Buy Candy Blossom (Robux) [BETA]",
            Description = "At Your Own Risk ! (Only Working in AWP EXECUTOR)",
            Callback = function()
                SendNotification("Some Executor may not supported", 1)
                local ReplicatedStorage_upvr = game:GetService("ReplicatedStorage")
                local MarketController_upvr = require(ReplicatedStorage_upvr.Modules.MarketController)
                local id = 3268187638
                MarketController_upvr:PromptPurchase(id, Enum.InfoType.Product)
            end
        })

        Tabs.Experimential:AddButton({
            Title = "Buy Exotic Seed Pack (Robux) [BETA]",
            Description = "At Your Own Risk ! (Only Working in AWP EXECUTOR)",
            Callback = function()
                SendNotification("Some Executor may not supported", 1)
                local ReplicatedStorage_upvr = game:GetService("ReplicatedStorage")
        ...
        local StockChecker = Tabs.Experimential:AddDropdown("StockChecker", {
            Title = "Stock Availible",
            Values = {},
            Multi = false,
            Default = "",
        ...
        Tabs.Experimential:AddButton({
            ...
            Callback = function()
                ...
                local DataService_upvr = require(ReplicatedStorage.Modules.DataService)
                local any_GetData_result1 = DataService_upvr:GetData()
                local event = any_GetData_result1.EventShopStock.Stocks
                local stock = {}
                for key, value in pairs(event) do
                    table.insert(stock, key)
                end
                StockChecker:SetValues(stock)
            end
        })

        local V_0_1
        local InputJobId2 = Tabs.Experimential:AddInput("InputJobId2", {
            Title = "Seed Name",
            Numeric = false,
            Description = "Example : Candy Blossom",
            Callback = function(Value)
                if Value then
                    V_0_1 = Value
                end
        ...
        Tabs.Experimential:AddButton({
            ...
            Description = "",
            Callback = function()
                if not SimpleSpyExecuted and not oh then
                    GameEvents.BuyEventShopStock:FireServer(tostring(V_0_1))
                else
                    warn("Failed to require EasterShop module!")
                    GameEvents.BuyEasterStock:FireServer(tostring(V_0_1))
                end
            end
        })

        local AutoBuy22 = Tabs.Experimential:AddToggle("AutoBuy22", {Title = "Auto Buy All Stock", Default = false })
        AutoBuy22:OnChanged(function()
            Config.AutoBuyEaster = Options.AutoBuy22.Value
            local easterItems = {
                "Chocolate Carrot",
                "Red Lollipop",
                "Candy Sunflower",
            ...
            if Config.AutoBuyEaster then
                task.spawn(function()
                    while _G.NoLag and Config.AutoBuyEaster do
                        if not SimpleSpyExecuted and not oh then
                            if workspace:GetAttribute("BloodMoonEvent") then
                                for _, item in pairs(easterItems) do
                                    local ReplicatedStorage = game:GetService("ReplicatedStorage")
                                    local DataService_upvr = require(ReplicatedStorage.Modules.DataService)
                                    local any_GetData_result1 = DataService_upvr:GetData()
                                    local event = any_GetData_result1.EventShopStock.Stocks
                                    for key, value in pairs(event) do
                                        if item == tostring(key) then
                                            for i = 1, value.MaxStock do
                                                GameEvents.BuyEventShopStock:FireServer(item)
                                                task.wait()
                                            end
                                        end
                                    end
        ...
        local AutoBuy23 = Tabs.Experimential:AddToggle("AutoBuy23", {Title = "Auto Buy All (Xeno/Solara Executor)", Default = false })
        AutoBuy23:OnChanged(function()
            Config.AutoBuyEaster2 = Options.AutoBuy23.Value
            local easterItems = {
                "Chocolate Carrot",
                "Red Lollipop",
                "Candy Sunflower",
            ...
            if Config.AutoBuyEaster2 then
                task.spawn(function()
                    while _G.NoLag and Config.AutoBuyEaster2 do
                        if workspace:GetAttribute("BloodMoonEvent") then
                            if not SimpleSpyExecuted and not oh then
                                for _, item in pairs(easterItems) do
                                    GameEvents.BuyEventShopStock:FireServer(item)
        ...
        local function checkAlarm()
            ...
            local alarmActive = (minutes >= 1 and minutes <= 10)

            return alarmActive
        end
        local HopMoon = Tabs.Experimential:AddToggle("HopMoon", {Title = "Hop Server until Blood Moon", Description = "Only active in **:00 - **:05",Default = false })
        HopMoon:OnChanged(function()
            Config.HopMoon = Options.HopMoon.Value
            if Config.HopMoon then
                task.spawn(function()
                    while _G.NoLag and Config.HopMoon do
                        if checkAlarm() then
                            print("Teleporting.....")
                            task.wait(5)
                            if not workspace:GetAttribute("BloodMoonEvent") or workspace:GetAttribute("BloodMoonEvent") == false then
                                BaseModule:hopServer()
                            end
                        end
                        task.wait(10)
