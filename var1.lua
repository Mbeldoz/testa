...
local connections = {} do
    ...
    connections.notifFrame = notifFrame.ChildAdded:Connect(function(child)
        if _G.highUNC then
            if child.Name == "Notification_UI" then
                local text = child.TextLabel.Text
                local text_lower = text:lower()
                if text_lower:find("too close") then
                    Config.childnotif = child.TextLabel
                    Config.Tooclose = true
                end
            end
        end
    end)
